            ...
            <div className="grid grid-cols-3 gap-4">
                {flyerData.map((item, idx) => {
                    ...
                    if (item.isValid == "FALSE" || showAllFlyers == "TRUE") {
                        return <FlyerCard key={idx} imageData={item.imageData} org={item.org} date={item.date} loc={item.loc} pointOfContact={item.pointOfContact} pointOfContactName={item.pointOfContactName} cate1={item.cate1} cate2={item.cate2} cate3={item.cate3} link={item.link} filename={item.filename} mongoid={item._id} />
                    }
                })}
            </div>
        </div>
